...
// Source File Name:   HrmScheduleDiffDetBeLateManager.java
...
public class HrmScheduleDiffDetBeLateManager extends BaseBean {
	...
	public List getScheduleList(String s, String s1, int i, int j, int k)
	{
		...
		while (!flag2) 
		{
			...
			} else
			{
				...
				stringbuffer.append("   from HrmScheduleSign a,HrmResource b ").append("  where a.userId=b.id ").append("    and a.signDate='").append(s24).append("'").append("    and a.signType='1' ").append("    and a.isInCom='1' ").append("    and a.userType='1' ").append("    and a.signTime<'").append(s23).append(":00' ");
				if (i > 0)
				...
				stringbuffer.append(" )TempTable where signTime>'").append(s22).append(":00' ");
				...
				do
				{
					...
					boolean flag1 = getHasApproved(Util.getIntValue(s9, -1), s24, s22, s21);
					if (!flag1)
					{
	...
	private boolean getHasApproved(int i, String s, String s1, String s2) {
		boolean flag = false;
		String s3 = getNextMinOfSignTime((new StringBuilder()).append(s)
				.append(" ").append(s2).toString());
		RecordSet recordset = new RecordSet();
		StringBuffer stringbuffer = new StringBuffer();
		stringbuffer.append(" select 1 ").append(
				"   from Workflow_Requestbase a,Bill_BoHaiLeave b ").append(
				"  where a.requestId=b.requestId ").append(
				"    and a.currentNodeType='3' ").append("    and resourceId=")
				.append(i);
		if (recordset.getDBType().equals("oracle"))
			stringbuffer.append(
					"    and concat(concat(fromDate,' '),fromTime)<='").append(
					s).append(" ").append(s1).append("' ").append(
					"    and concat(concat(toDate,' '),toTime)>='").append(s3)
					.append("' ");
		else
			stringbuffer.append("    and fromDate+' '+fromTime<='").append(s)
					.append(" ").append(s1).append("' ").append(
							"    and toDate+' '+toTime>='").append(s3).append(
							"' ");
		recordset.executeSql(stringbuffer.toString());
		if (recordset.next()) {
			flag = true;
		...
		stringbuffer.append(" select 1 ").append(
				"   from Workflow_Requestbase a,Bill_BoHaiEvection b ").append(
				"  where a.requestId=b.requestId ").append(
				"    and a.currentNodeType='3' ").append("    and resourceId=")
				.append(i);
		if (recordset.getDBType().equals("oracle"))
		...
		stringbuffer = new StringBuffer();
		stringbuffer.append(" select 1 ").append(
				"   from Workflow_Requestbase a,Bill_BoHaiOut b ").append(
				"  where a.requestId=b.requestId ").append(
				"    and a.currentNodeType='3' ").append("    and resourceId=")
				.append(i);
		if (recordset.getDBType().equals("oracle"))
	...
	private String getNextMinOfSignTime(String s) {
		String s1 = "";
